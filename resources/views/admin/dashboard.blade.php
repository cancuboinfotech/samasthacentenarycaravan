        ...
        <div class="p-6 text-gray-900">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="bg-blue-50 p-6 rounded-lg">
                    ...
                    <p class="text-3xl font-bold text-blue-600">{{ $caravansCount }}</p>
                </div>
                
                <div class="bg-green-50 p-6 rounded-lg">
                    <h3 class="text-lg font-semibold text-green-900 mb-2">Active Destinations</h3>
                    <p class="text-3xl font-bold text-green-600">{{ $destinationsCount }}</p>
                </div>
            </div>
            ...
            <div class="mb-6">
                ...
                <div class="flex flex-wrap gap-4">
                    ...
                    <a href="{{ route('admin.destinations.index') }}" class="bg-gray-600 text-white px-4 py-2 rounded hover:bg-gray-700">
                        Manage Destinations
                    </a>
                    <a href="{{ route('map.index') }}" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">
                        View Map
                    </a>
                </div>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($recentCaravans as $caravan)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        {{ $caravan->name }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $caravan->vehicle_number }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        @if($caravan->latestLocation)
                                            {{ $caravan->latestLocation->city ?? 'N/A' }}, {{ $caravan->latestLocation->state ?? 'N/A' }}
                                        @else
                                            No location data
                                        @endif
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <a href="{{ route('map.show', $caravan) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
